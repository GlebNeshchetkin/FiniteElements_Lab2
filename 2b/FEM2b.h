...
double FEM<dim>::basis_function(unsigned int node, double xi_1, double xi_2, double xi_3){
  ...
  if (node==0) { value = (1-xi_1)*(1-xi_2)*(1-xi_3) / 8; }
  if (node==1) { value = (1+xi_1)*(1-xi_2)*(1-xi_3) / 8; }
  if (node==2) { value = (1-xi_1)*(1+xi_2)*(1-xi_3) / 8; }
  if (node==3) { value = (1+xi_1)*(1+xi_2)*(1-xi_3) / 8; }
  if (node==4) { value = (1-xi_1)*(1-xi_2)*(1+xi_3) / 8; }
  if (node==5) { value = (1+xi_1)*(1-xi_2)*(1+xi_3) / 8; }
  if (node==6) { value = (1-xi_1)*(1+xi_2)*(1+xi_3) / 8; }
  if (node==7) { value = (1+xi_1)*(1+xi_2)*(1+xi_3) / 8; }
  //EDIT
  return value;
}
...
void FEM<dim>::define_boundary_conds(){
  ...
  for (uint i = 0; i < totalNodes ; ++i) {
    if (std::abs(nodeLocation[i][0])<1.e-8) {
      boundary_values[i] = 300*(1+1/3*(nodeLocation[i][1]+nodeLocation[i][2]));
    }
    if (std::abs(nodeLocation[i][0]-0.04)<1.e-8) {
                ...
                for(unsigned int j=0;j<dim;j++) {
                  for(unsigned int I=0;I<dim;I++) {
                    for(unsigned int J=0;J<dim;J++) {
                      ...
                      Klocal[A][B] += basis_gradient(A,quad_points[q1],quad_points[q2],quad_points[q3])[i] * invJacob[i][I] * kappa[I][J] * basis_gradient(B,quad_points[q1],quad_points[q2],quad_points[q3])[j] * invJacob[j][J] * quad_weight[q1]*quad_weight[q2]*quad_weight[3]*detJ;
                    }
                  }
                }
              }
